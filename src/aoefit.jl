...
###############################################################
# choose between one and two EMG background function
###############################################################
...
function fit_single_aoe_compton(h::Histogram, ps::NamedTuple; uncertainty::Bool=true, pseudo_prior::NamedTupleDist=NamedTupleDist(empty = true), fit_func::Symbol=:aoe_one_bck, background_center::Union{Real,Nothing} = ps.peak_pos, fixed_position::Bool=false)
    ...
    ###v_init = Vector(mean(f_trafo.target_dist)) change here acoording to old code
    v_init = mean(pseudo_prior)

    ### suggestion of co pilot
    ###v_init_array = collect(values(f_trafo(v_init)))
    ###opt_r = optimize((-) ∘ f_loglike ∘ inverse(f_trafo), v_init_array, Optim.Options(time_limit = 60, iterations = 3000))
    ...
    # MLE
    ###opt_r = optimize((-) ∘ f_loglike ∘ inverse(f_trafo), v_init, Optim.Options(time_limit = 60, iterations = 3000)) needed to change this
    ...
    if !converged @warn "Fit did not converge (single aoe compton)" end
