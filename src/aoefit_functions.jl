...
mod_f_aoe_compton(x, v) = mod_aoe_compton_peakshape(x, v.μ, v.σ, v.n, v.B, v.δ, v.μ2, v.σ2, v.B2, v.δ2)
mod_f_aoe_bkg(x, v) = mod_aoe_compton_background_peakshape(x, v.μ, v.σ, v.B, v.δ, v.μ2, v.σ2, v.B2, v.δ2)
...
###############################################################
# modified version with two EMGs
###############################################################

function mod_get_aoe_fit_functions(; background_center::Union{Real,Nothing} = nothing)
    merge( 
        (f_fit = (x, v) -> mod_aoe_compton_peakshape(x, v.μ, v.σ, v.n, v.B, v.δ, v.μ2, v.σ2, v.B2, v.δ2),),
    if isnothing(background_center)
        NamedTuple()
    else
        NamedTuple()
...
These component functions are used in the fit-report and in plot receipes 
"""
function mod_aoe_compton_peakshape_components(fit_func::Symbol; background_center::Union{Real,Nothing} = nothing)
    if fit_func == :f_fit
        funcs = (f_sig = (x, v) -> aoe_compton_signal_peakshape(x, v.μ, v.σ, v.n),
            f_bck = (x, v) -> mod_aoe_compton_background_peakshape(x, v.μ, v.σ, v.B, v.δ, v.μ2, v.σ2, v.B2, v.δ2))
    end 
    labels = (f_sig = "Signal", f_bck = "Background")
    colors = (f_sig = :orangered1, f_bck = :dodgerblue2)
    linestyles = (f_sig = :solid, f_bck = :dash)
...
function mod_aoe_compton_peakshape_components(fit_func::Symbol, v::NamedTuple; background_center::Union{Real,Nothing} = v.μ)
    components  = mod_aoe_compton_peakshape_components(fit_func; background_center = background_center)
    out = (; components..., funcs = merge([NamedTuple{Tuple([name])}(Tuple([x -> Base.Fix2(components.funcs[name], v)(x)]))  for name in  keys(components.funcs)]...))
    return out
end